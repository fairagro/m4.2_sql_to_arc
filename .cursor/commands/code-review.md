...
**Output:** write a report under `/tmp` via `m42-ai` (body starts with `<!-- m42-ai:code-review -->` + findings
table); for a PR, publish a formal COMMENT Pull Request Review when auth works. Do not triage Copilot/Bugbot threads
(use `/review-fixer`). Do not commit, push, or auto-approve.
