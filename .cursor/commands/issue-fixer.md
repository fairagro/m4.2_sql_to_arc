...
→ apply → pause → draft PR → archive (last `go`), **except** a clearly docs-only slice (Markdown/comments, no skill
file) which uses the Bug path. **Task / Bug / cheap Security:** issue branch → implement → pause → draft PR (no OpenSpec
unless asked or a skill file is in scope). Do **not** auto-commit fix commits. Do **not** use empty bootstrap commits;
open the draft PR only after real commits exist ahead of `main`. `/review-fixer` and `/create-issue` do **not** run
OpenSpec.
...
4. OpenSpec types: follow `.cursor/skills/openspec-propose/SKILL.md` → pause → on `go` follow `openspec-apply-change` →
   pause. Task/Bug path **or** clearly docs-only Feature/Refactoring (Markdown/comments, no skill file): implement in
   the working tree → pause.
