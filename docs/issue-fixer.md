...
Shared `/issue-fixer` triages a GitHub issue, explores when needed, and implements on `issue-<n>-<slug>`. **Task /
Feature / Refactoring** use OpenSpec (propose → apply → draft PR → archive as last `go`), except a **clearly docs-only**
slice (Markdown/MDC and/or code comments — **not** if a skill file is touched). **Bug** and cheap **Security** stay on
the fast path (no OpenSpec unless asked). No auto-commit of fix commits; draft PR only after real commits exist.
Canonical skill: [`.agents/skills/issue-fixer/SKILL.md`](../.agents/skills/issue-fixer/SKILL.md). Devinfra is source of
truth — products must not fork the skill (sync allowlist).
...
6. **`skip_specs`:** when OpenSpec **is** used, real delta spec if a capability contract changes; `skip_specs: true`
   ...
   without a skill file skips OpenSpec entirely rather than using `skip_specs`.
