...
| **Env / CI inputs / wrappers**   | Config file has no merge/`extends` — keep the synced blob generic                        | `MYPYPATH`, reusable `pylint_source_roots`, CST bake env, optional `.devcontainer/product.env` (Compose `env_file`, not synced)                                                                                                                                                                                                                                                                                                                                                          |
| **Verbatim shared fragment**     | Fleet wants identical policy; product deltas are wrong or go upstream                    | `ruff.toml` ([#60](https://github.com/fairagro/m4.2_middleware_devinfra/issues/60)), `pyrightconfig.json` ([#64](https://github.com/fairagro/m4.2_middleware_devinfra/issues/64)), `.vscode/settings.json`, `.devcontainer/devcontainer.json` + `docker-compose.yml` (`/workspace` + basename `name`/volumes — [#65](https://github.com/fairagro/m4.2_middleware_devinfra/issues/65)), `.pre-commit-config.yaml` ([#63](https://github.com/fairagro/m4.2_middleware_devinfra/issues/63)) |
