...
| **Verbatim shared fragment**        | Fleet wants identical policy; product deltas are wrong or go upstream                    | `ruff.toml` (no product `extend` / ignore overlay — see [#60](https://github.com/fairagro/m4.2_middleware_devinfra/issues/60)), `pyrightconfig.json` (venv + `stubPath` + `scripts/ai` only — see [#64](https://github.com/fairagro/m4.2_middleware_devinfra/issues/64)), `.vscode/settings.json` (empty `pytestArgs`; test roots in each repo’s `pyproject.toml` `testpaths`) |
| **Product-owned until split lands** | Synced blob is not yet generic enough; interim = not an overwrite target                 | `.pre-commit-config.yaml` / `devcontainer.json` ownership stories: [#63](https://github.com/fairagro/m4.2_middleware_devinfra/issues/63), [#65](https://github.com/fairagro/m4.2_middleware_devinfra/issues/65)                                                                                                                                                                |
